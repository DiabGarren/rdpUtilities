...
export function setProfile(firstName, lastName) {
  document.querySelector(
    "#profile"
  ).textContent = `${firstName[0]}${lastName[0]}`;
}
...
export async function createWardCouncilDoc(
  date,
  opening,
  st,
  train,
...
export async function updateWardCouncilDoc(
  date,
  opening,
  st,
  train,
...
export async function createBishopricDoc(
  date,
  opening,
  st,
  train,
...
export async function updateBishopricDoc(
  date,
  opening,
  st,
  train,
...
export async function createSacramentDoc(
  date,
  conducting,
  announcements,
  openingPrayer,
...
export async function updateSacramentDoc(
  date,
  conducting,
  announcements,
  openingPrayer,
  ...
  const dropdown = () => {
    ...
    for (const page of pages) {
      if (page[1] === currPage) {
        ...
        ((page[0] === "bishopric" || page[0] === "sacrament") &&
          level < BISHOPRIC)
      ) {
        output += "";
      } else {
...
export async function renderBasepage(meeting, userData, getAllFunc, wrapper) {
  ...
  docs.forEach((doc) => {
    ...
    wcDoc.textContent = `${docDate.getDate()} ${docDate.toLocaleString(
      "default",
      { month: "long" }
    )} ${docDate.getFullYear()}`;
...
export async function renderDocPage(
  ...
  document.title += ` - ${docDate.getDate()} ${docDate.toLocaleString(
    "default",
    { month: "short" }
  )} ${docDate.getFullYear()}`;
  ...
  if (doc.length > 0) {
    const meetingType = () => {
      if (meeting.toLowerCase() === "bishopric") {
        ...
        return `Roodepoort Ward<br>
                ...
                7:00pm ${docDate.getDate()} ${docDate.toLocaleString(
          "default",
          { month: "long" }
        )} ${docDate.getFullYear()}<br>
    ...
    if (doc[0].createdBy.date !== "n/a") {
      ...
      output += ` - ${createdDate.getDate()} ${createdDate.toLocaleString(
        "default",
        { month: "long" }
    ...
    if (doc[0].updatedBy[doc[0].updatedBy.length - 1].date !== "n/a") {
      const createdDate = new Date(
        doc[0].updatedBy[doc[0].updatedBy.length - 1].date
      );
      output += ` - ${createdDate.getDate()} ${createdDate.toLocaleString(
        "default",
        { month: "long" }
...
export async function renderNewDocPage(
  meeting,
  userData,
  createDocFunc,
  wrapper,
...
export async function renderUpdateDocPage(
  meeting,
  userData,
  getDocFunc,
  updateDocFunc,
  ...
  document.querySelector(
    "#back"
  ).href = `/rdpUtilities/${meeting}/?date=${doc[0].date}`;
    ...
    output += `
        ...
        <table class="print-font">
            ...
            <tr>
                ...
                <td>
                    <hi>Date:</hi>
                    <pi>${docDate.getDate()} ${docDate.toLocaleString(
      "default",
      { month: "long" }
    )} ${docDate.getFullYear()}</pi>
                </td>
            </tr>
            <tr>
                <td colspan=2>
        ...
        <a class="btn btn-green no-print" href="/rdpUtilities/sacrament/?date=${
          doc[0].date
        }&update=true">Update Document</a>
        ${deleteDoc()}
...
export async function renderUpdateSacrament(
  getDocFunc,
  updateDocFunc,
  date,
  wrapper,
  ...
  addItem(
    document.querySelector("#release-list"),
    doc[0].wardBusiness.releases
  );
  addItem(
    document.querySelector("#sustain-list"),
    doc[0].wardBusiness.sustainings
  );
  ...
  document.querySelector(
    "#back"
  ).href = `/rdpUtilities/sacrament/?date=${doc[0].date}`;
  ...
  document.querySelectorAll(".assignment-check").forEach((element) => {
    element.addEventListener("click", async (event) => {
      const res = await updateAssignment(
        ...
        event.target.dataset.assignment,
...
export async function renderNewYouthActivity(wrapper) {
  ...
  document.querySelector("#submit").addEventListener("click", async () => {
    ...
    const response = await createYouthActivity(
      date.value,
      combined.checked,
      activity
    );
